...
rune utf8_decode(const char **char_str) {
    u8 **s = (u8 **)char_str;
...
usize utf8_encode(char *str, rune codepoint) {
    usize len = 0;
    u8 first;

    u32 ch = codepoint;

    if (ch < 0x80) {            // 0000-0000 0000-0000 0000-0000 1000-0000
        first = 0;
        len = 1;
    }
    else if (ch < 0x800) {      // 0000-0000 0000-0000 0000-1000 0000-0000
        first = 0xc0; // 1100-0000
        len = 2;
    }
    else if (ch < 0x10000) {    // 0000-0000 0000-0001 0000-0000 0000-0000
        first = 0xe0; // 1110-0000
        len = 3;
    }
    else {
...
int utf8_size(const char *str) {
    u8 c = (u8)*str;
    constexpr usize arrlen = sizeof(sizes) / sizeof(*sizes);
...
rune utf8_get(FILE *f) {
    ...
    // read first byte of codepoint from file
    int c = fgetc(f);
    if (c == EOF) {
        return utf8_invalid;
    }
    // save that byte into the buffer
    buffer[0] = (char)c;
    ...
    if (size > 1) {
        // read rest of the codepoint
        int amount = fread(&buffer[1], 1, size - 1, f);
        if (amount != size - 1) {
            return utf8_invalid;
        }
    }
